...
from SVE_module_dynamic_uh import SVE as SVE_standard
from SVE_module_dynamic_uh_mff_ts import SVE as SVE_mff
...
if __name__ == "__main__": 
    ...
    """
    hdf_filename = '/qfs/people/feng779/PYTHON/ML/PINN_SVE/HEC-RAS/case4/MixedFlow.p02.hdf'
    ...
    weight_path = 'log/case4/weights.out'
    # Training
    model1 = SVE_standard(X_u_IC, X_h_IC,
                X_u_BC, X_h_BC,
                X_u_obs, X_h_obs,
                X_f_train,
                u_IC, h_IC,
    ...
    weight_path = 'log/case4_mff/weights.out'
    wmff_path = 'saved_model/case4_mff/w_mff.out'
    # Training
    model2 = SVE_mff(X_u_IC, X_h_IC,
                X_u_BC, X_h_BC,
                X_u_obs, X_h_obs,
                X_f_train,
                u_IC, h_IC,
    ...
    for k in range(len(tlist)):
        ...
        axes[k, 0].plot(x, u_pred1[tlist[k],:], '-b', linewidth=2, label='PINN (standard)')
        axes[k, 0].plot(x, u_pred2[tlist[k],:], '-r', linewidth=2, label='PINN (ff)')
        axes[k, 0].text(0.85, 0.85, 't={} h'.format(int(tlist[k])), fontsize=15, transform=axes[k,0].transAxes)
        axes[k, 0].set_ylabel('Velocity (m/s)')
        axes[k, 0].set_ylim([0,4])
        axes[k, 0].grid()
        axes[k, 1].plot(x, h_test[tlist[k],:]+eles, '-k', linewidth=2, label='reference')
        axes[k, 1].plot(x, h_pred1[tlist[k],:]+eles, '-b', linewidth=2, label='PINN (stardard)')
        axes[k, 1].plot(x, h_pred2[tlist[k],:]+eles, '-r', linewidth=2, label='PINN (ff)')
