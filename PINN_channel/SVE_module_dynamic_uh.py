...
class SVE:
    ...
    def __init__(self, X_u_IC, X_h_IC, 
        ...
        if ExistModel == 0:
            ...
            if self.useObs:
                ...
                print("constant_bcs_u_val: {:.3f}, constant_bcs_h_val: {:.3f}, constant_ics_u_val: {:.3f}, constant_ics_h_val: {:.3f}".format(self.adaptive_constant_bcs_u_tf, self.adaptive_constant_bcs_h_tf, self.adaptive_constant_ics_u_tf, self.adaptive_constant_ics_h_tf))
        ...
        self.dict_gradients_ics_u_layers = self.generate_grad_dict(self.layers)
    ...
    def callback_obs(self, loss, loss_f_c, loss_f_m, loss_BC_u, loss_BC_h, loss_IC_u, loss_IC_h, loss_obs_u, loss_obs_h):
    ...
    def callback(self, loss, loss_f_c, loss_f_m, loss_BC_u, loss_BC_h, loss_IC_u, loss_IC_h):
        self.count = self.count+1
        print('{} th iterations, Loss: {:.3e}, Loss_f_c: {:.3e}, Loss_f_m: {:.3e}'.format(self.count, loss, loss_f_c, loss_f_m))
        self.loss_f_c_log.append(loss_f_c)
        self.loss_f_m_log.append(loss_f_m)
    ...
    def train(self, num_epochs):
        ...
        for it in range(num_epochs):
            ...
            if it % 10 == 0:
                ...
                if self.useObs: 
                    loss_BC_u, loss_BC_h, loss_IC_u, loss_IC_h, loss_obs_u, loss_obs_h, loss_f_c, loss_f_m = \
                        self.sess.run([self.loss_BC_u, self.loss_BC_h, self.loss_IC_u, self.loss_IC_h, self.loss_obs_u, self.loss_obs_h, self.loss_f_c, self.loss_f_m], tf_dict)
                    print ('Loss_BC_u: %.3e, Loss_BC_h: %.3e, Loss_IC_u: %.3e, Loss_IC_h: %.3e, Loss_obs_u: %.3e, Loss_obs_h: %.3e, Loss_f_c: %.3e, Loss_f_m: %.3e, Error u: %.3e, Error h: %.3e'
                            %(loss_BC_u, loss_BC_h, loss_IC_u, loss_IC_h, loss_obs_u, loss_obs_h, loss_f_c, loss_f_m, error_u, error_h))
                else:
