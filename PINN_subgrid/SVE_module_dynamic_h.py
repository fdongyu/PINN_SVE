...
class SVE:
    ...
    def __init__(self, X_h_IC, 
        ...
        if ExistModel == 0:
            ...
            print("constant_bcs_u_val: {:.3f}, constant_bcs_h_val: {:.3f}, constant_ics_h_val: {:.3f}, constant_obs_h_val: {:.3f}".format(self.adaptive_constant_bcs_u_tf, self.adaptive_constant_bcs_h_tf, self.adaptive_constant_ics_h_tf, self.adaptive_constant_obs_h_tf))
        ...
        if self.useObs:
            ...
            self.loss_obs = self.adaptive_constant_obs_h_tf * self.loss_obs_h
    ...
    def save_gradients(self, tf_dict):
        ...
        for i in range(num_layers - 1):
            if self.useObs:
                grad_res_value, grad_bcs_u_value, grad_bcs_h_value, grad_ics_h_value, grad_obs_h_value = \
                self.sess.run([self.grad_res[i], self.grad_bcs_u[i], self.grad_bcs_h[i], self.grad_ics_h[i], self.grad_obs_h[i]], feed_dict=tf_dict)
            else:
    ...
    def net_f(self, x_f, t_f):
        ...
        u = self.neural_net(X_f, self.weights, self.biases)[:, 0:1]
        u_t = tf.gradients(u, t_f)[0]
        u_x = tf.gradients(u, x_f)[0]
        
        h = self.neural_net(X_f, self.weights, self.biases)[:, 1:2]
    ...
    def callback(self, loss, loss_f_c, loss_f_m, loss_BC_u, loss_BC_h, loss_IC_h, loss_obs_h):
    ...
    def train(self, num_epochs):
        ...
        for it in range(num_epochs):
            ...
            if it % 10 == 0:
                ...
                if self.useObs: 
                    loss_BC_u, loss_BC_h, loss_IC_h, loss_obs_h, loss_f_c, loss_f_m = \
                        self.sess.run([self.loss_BC_u, self.loss_BC_h, self.loss_IC_h, self.loss_obs_h, self.loss_f_c, self.loss_f_m], tf_dict)
                    print ('Loss_BC_u: %.3e, Loss_BC_h: %.3e, Loss_IC_h: %.3e, Loss_obs_h: %.3e, Loss_f_c: %.3e, Loss_f_m: %.3e, Error u: %.3e, Error h: %.3e'
                            %(loss_BC_u, loss_BC_h, loss_IC_h, loss_obs_h, loss_f_c, loss_f_m, error_u, error_h))
                else:
                ...
                if self.useObs:
                    print("constant_bcs_u_val: {:.3f}, constant_bcs_h_val: {:.3f}, constant_ics_h_val: {:.3f}, constant_obs_h_val: {:.3f}".format(self.adaptive_constant_bcs_u_val, self.adaptive_constant_bcs_h_val, self.adaptive_constant_ics_h_val, self.adaptive_constant_obs_h_val))
                else:
    ...
    def train_bfgs(self):
        ...
        if self.useObs:
            self.optimizer.minimize(self.sess,
                                    feed_dict=tf_dict,
                                    fetches=[self.loss, self.loss_f_c, self.loss_f_m, self.loss_BC_u, self.loss_BC_h, self.loss_IC_h, self.loss_obs_h],
